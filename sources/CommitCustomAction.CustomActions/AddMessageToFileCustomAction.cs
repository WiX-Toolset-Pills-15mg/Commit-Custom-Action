...
{
    public class AddMessageToFileCustomAction
    {
        [CustomAction("AddMessageToFile")]
        public static ActionResult Execute(Session session)
        {
            return CustomActionExecution.Run(session, () =>
            {
                ...
                string backupFilePath = thatOneFile.CreateBackup();
                session.Log($"Backup file was successfully created: '{backupFilePath}'.");

                thatOneFile.AddMessage();
                session.Log($"New message was successfully added into file '{filePath}'.");
            });
        }
    }
}